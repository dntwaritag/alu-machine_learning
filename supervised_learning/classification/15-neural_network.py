...
class NeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,
        ...
        for i in range(iterations + 1):
            A1, A2 = self.forward_prop(X)  # Perform forward propagation
            self.gradient_descent(X, Y, A1, A2, alpha)  # Update weights and bias

            # Every `step` iterations, record cost and optionally print/log it
            if i % step == 0 or i == iterations:
                cost = self.cost(Y, A2)  # Calculate current cost
                costs.append(cost)
